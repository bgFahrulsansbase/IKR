    ...
    if ($("#kelas").val() == "" || $("#menanya").val() == "") {
      ...
      getDataSiswa.then((users) => {
        ...
        for (let user in users) {
          if (users[user].menanya) {
            if (users[user].kelas == $("#kelas option:selected").text()) {
              ...
              <td class="text-jwbsiswa">Pertanyaan: ${
                users[user].menanya[$("#menanya option:selected").val()]
                  .pertanyaan1 || "-"
              } <br>Jawaban: ${
                users[user].menanya[$("#menanya option:selected").val()]
                  .jawaban1 || "-"
        ...
        $("#tabelData").on("change", ".nilai-input", function () {
          ...
          const topikId = $("#menanya").val();

          const userRef = ref(db, `UserSiswa/${userId}/menanya/${topikId}`);
      ...
      $(".detailMenanya").html($("#menanya").find("option:selected").text());
    }
  });

  function menampilkan(fixKelas, fixMenanya) {}
});
